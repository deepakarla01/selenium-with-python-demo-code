...
def on_textbox_page_is_successful(textbox_setup, url):
    assert textbox_setup.go_to_textbox(
        url
    ), "An unexpected error occurred in go_to_textbox"
...
def entering_full_name(textbox_setup, full_name):
    assert textbox_setup.enter_full_name(
        full_name
    ), "An unexpected error occurred in enter_full_name"
...
def entering_e_mail(textbox_setup, e_mail):
    assert textbox_setup.enter_email(
        e_mail
    ), "An unexpected error occurred in enter_e_mail"
...
def entering_current_address(textbox_setup, current_address):
    assert textbox_setup.enter_current_address(
        current_address
    ), "An unexpected error occurred in enter_current_address"
    log.info(f'When I enter a value "{current_address}" in the Current Address field')


@when(
    parsers.parse(
        'I enter a value "{permanent_address}" in the Permanent Address field'
    )
)
def entering_permanent_address(textbox_setup, permanent_address):
    assert textbox_setup.enter_permanent_address(
        permanent_address
    ), "An unexpected error occurred in enter_permanent_address"
    log.info(
        f'When I enter a value "{permanent_address}" in the Permanent Address field'
    )
...
def clicking_submit_button(textbox_setup):
    assert (
        textbox_setup.click_submit()
    ), "An unexpected error occurred in click_submit_button"
...
def successful_submit(textbox_setup):
    assert (
        textbox_setup.success_submit()
    ), "An unexpected error occurred in success_submit"
