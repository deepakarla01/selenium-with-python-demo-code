...
def i_click_on_menu_link(menu_link, elements_setup, url):
    navigation_map = {
        ...
        "Broken Links - Images": lambda: elements_setup[0].click_broken_links_images(
            url
        ),
    ...
    try:
        for key, value in navigation_map.items():
            ...
            log.error(
                f"When: No matching click function found for menu link: {menu_link}"
            )
...
def i_should_navigate_to_menu_link_page(menu_link, elements_setup, url):
    navigation_map = {
        ...
        "Broken Links - Images": lambda: elements_setup[
            0
        ].navigate_to_broken_links_images_page(url),
        "Upload and Download": lambda: elements_setup[
            0
        ].navigate_to_upload_and_download_page(url),
        "Dynamic Properties": lambda: elements_setup[
            0
        ].navigate_to_dynamic_properties_page(url),
    }
    ...
    try:
        for key, value in navigation_map.items():
            ...
            log.error(
                f"Then: No matching navigation function found for menu link: {menu_link}"
            )
    except Exception as e:
        log.error(
            f"Then: An unexpected error occurred in i_should_navigate_to_menu_link_page: {e}"
        )
        raise
