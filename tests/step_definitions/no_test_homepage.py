...
def click_on_elements_card_is_successful(card_name, homepage_setup):
    ...
    try:
        for key, value in navigation_map.items():
            ...
            log.error(
                f"Then: No matching click function found for card link: {card_name}\n"
            )
    except Exception as e:
        log.error(
            f"Then: An unexpected error occurred in click_on_elements_card_is_successful: {e}\n"
        )
        raise
...
def navigate_to_elements_page_is_successful(card_name, homepage_setup, url):
    navigation_map = {
        ...
        "Alerts_Frame_Windows": lambda: homepage_setup.navigate_to_alerts_frame_window_page(
            url
        ),
    ...
    try:
        for key, value in navigation_map.items():
            ...
            log.error(
                f"Then: No matching navigation function found for card link: {card_name}\n"
            )
    except Exception as e:
        log.error(
            f"Then: An unexpected error occurred in navigate_to_elements_page_is_successful: {e}\n"
        )
        raise
